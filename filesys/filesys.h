...
class FileSystem {
  public:
    ...
    OpenFile* Open(char *name) {
		  ...
		  //cout<<"open filedes: "<<fileDescriptor<<endl;
	...
	int OpenAFile(char *name) {
		//cout<<"hi!\n";
		//int fileDes = OpenForReadWrite(name, FALSE);
        	filePtr = Open(name);

		if(filePtr!=NULL)// open successed
			return 1;
		else
			return -1;
	}
	int WriteFile(char *buffer, int size) {
		if(filePtr==NULL)
			...
			int num = filePtr->Write(buffer, size);
			if(num>0)
				return num;
			else
				return -1;	
	...
	int ReadFile(char *buffer, int size) {
		if(filePtr==NULL)
			...
			int num = filePtr->Read(buffer, size);
			if(num>0)
				return num;
			else
				return -1;	
